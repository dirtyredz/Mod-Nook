...
└─ src/
   ...
   ├─ ui/                    the panel we draw. Only the VENDORED primitives sit at this level —
   │  │                      they are byte-synced with sibling mods, so their path is fixed
   │  ├─ PanelSprite.cs      procedural 9-sliced plate + circle sprites  (shared with 5 mods)
...
- `src/ui/` — vendored drawing primitives ONLY (`PanelSprite`, `Palette`); see the note below
...
`PanelSprite.cs` and `Palette.cs` deliberately stay at `src/ui/` itself: they are **vendored**, kept
byte-identical with 5 and 2 sibling mods respectively, and every other mod has them at `src/ui/<file>`.
Moving them into a subfolder here would make this repo the odd one out and break the diffability
`tools/sync-mod-files.ps1` depends on. If a third non-vendored file ever lands at `src/ui/` root, it
belongs in one of the two subfolders instead.
...
| **Catalog** | `core/ModCatalog.cs` (170), `core/Tags.cs` (46) | Discover loaded plugins that expose settings; group into `ModEntry`/`SectionEntry`; honour `ModNook.Hidden`. `Tags` reads the optional `ModNook.*` description tags. | BepInEx Chainloader | Discovery / tag vocabulary |
| **Drawing & theming** | `ui/PanelSprite.cs` (182), `game/GameFonts.cs` (178), `ui/Palette.cs` (10), `ui/chrome/Tooltip.cs` (184), `ui/chrome/UiText.cs` (40) | Procedural sliced/circle sprites; find & apply the game font; shared colours; hover tooltip + `TooltipTrigger`; `UiText` is the shared TMP-label + full-parent-stretch pair used by the panel chrome and content. | game UI/TMP | Look & feel primitives |
